...
Note that one issue we have to face when choosing an algorithm is that the p-value function
	...
	\begin{itemize}
		\item \pkg{soma} \citep{zelinka_soma:_2014}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\linewidth]{figure/BF_compare}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\linewidth]{figure/c-drift}
	\caption{}
	\label{fig:c-drift}
\end{figure}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\linewidth]{figure/ct-trend}
	\caption{}
	\label{fig:ct-trend}
\end{figure}
...
\bibliography{Neves_2016_MMC_Rpack}
\end{document}
